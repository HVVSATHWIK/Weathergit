document.addEventListener('DOMContentLoaded', function () {
            ...
            try {
                ...
                if (!response.ok) {
                    if (response.status === 503 && i < retries - 1) {
                        // If it's a 503 error and there are retries left, wait and retry
                ...
                if (i === retries - 1) throw error; // Throw error if no retries left
            }
        }
    }
    ...
    async function fetchWeather(lat, lon) {
        try {
            ...
            console.log(aiData);  // Log the response for debugging
